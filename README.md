...
| Flow Omni 1.1 Flash 720p 제공 서비스와 사용자 계정 | 실제 동영상 생성 |
...
- 기본 8초 클립 5개, 기준 이미지 실제 첨부, Flow 720p → 실제 1080p 업스케일 각각 저장
...
동작은 Flow Omni 1.1 Flash 720p 영상, 원리는 움직이는 2D 설명 도식으로 보여줘.
내가 선택한 음성 서비스와 목소리와 포함된 기본 배경음악을 사용해줘.
최종 MP4·썸네일과 유튜브 제목·설명을 함께 전달해줘.
```

기본값은 45초 세로 쇼츠, Flow Omni 1.1 Flash 720p 생성 → 실제 1080p 업스케일 저장, 최종 1080×1920/30fps, 사용자가 선택한 음성 환경의 음성 1.2배속, 배경 상자 없는 Pretendard Bold 자막입니다. 사용자의 지정값이 우선합니다. 서비스에서 실제 모델/해상도를 확인하며 다른 모델로 임의 대체하지 않습니다.

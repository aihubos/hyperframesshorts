...
| Flow Omni 1.1 Flash 360p 제공 서비스와 사용자 계정 | 실제 동영상 생성 |
...
- 주제 10개 제안 → 사용자 선택 → 첫 3초 훅·기승전결·영상 개수·비용 브리핑
- 기본 8초 클립 5개, 기준 이미지 실제 첨부, Flow 360p → 실제 720p 업스케일 각각 저장
- 기존 Hypit/Hyperframes 편집 프로젝트 재사용, 45초 세로 영상
- 기존에 승인한 음성과 BGM 유지, 발화에 맞춘 배경 없는 자막, 불필요한 AI 고지 화면 자막 제외
- 상단 파란 큰 제목 제외, 하단 자막 문장 마침표 제거, 자막은 조금 아래로 배치하되 하단 여백 유지
- 영상·생성 썸네일·제목·설명 전달, 제목에 관련 해시태그 1~2개와 #Shorts
- 예약 자동화는 별도 연결·승인 후 사용. [주제 제안·제작·예약 운영](references/shorts-workflow.md)
...
동작은 Flow Omni 1.1 Flash 360p 영상, 원리는 움직이는 2D 설명 도식으로 보여줘.
내가 선택한 음성 서비스와 목소리와 포함된 기본 배경음악을 사용해줘.
최종 MP4·썸네일과 유튜브 제목·설명을 함께 전달해줘.
```

기본값은 45초 세로 쇼츠, Flow Omni 1.1 Flash 360p 생성 → 실제 720p 업스케일 저장, 최종 1080×1920/30fps, 사용자가 선택한 음성 환경의 음성 1.2배속, 배경 상자 없는 Pretendard Bold 자막입니다. 사용자의 지정값이 우선합니다. 서비스에서 실제 모델/해상도를 확인하며 다른 모델로 임의 대체하지 않습니다.
...
**Aside 우선·백그라운드 작업**입니다. 스킬이 브라우저 연결 기능을 설치하는 것은 아닙니다. 최근 제작에서는 Aside CLI를 사용하는 Flow 제작 흐름과 720p·1080p 파일 저장 기록을 확인했습니다. 다른 컴퓨터에서도 가능한지는 해당 환경의 CLI와 연결 상태를 확인합니다. 지원되지 않으면 화면을 임의로 점유하거나 다른 브라우저로 전환하지 않고 필요한 선택만 안내합니다. [동작 기준](references/production.md#aside와-백그라운드-작업)을 참고하세요.
